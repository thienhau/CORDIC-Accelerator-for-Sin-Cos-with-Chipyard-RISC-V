...
// Tự định nghĩa một Config để tắt Monitor thủ công
class WithNoMonitors extends Config((site, here, up) => {
  case MonitorsEnabled => false
})

class WithCordicAccelerator extends Config((site, here, up) => {
  case BuildRoCC => up(BuildRoCC) ++ Seq(
    (p: Parameters) => {
      val cordic = LazyModule(new chipyard.example.CordicAccelerator(OpcodeSet.custom0)(p))
      cordic
    }
  )
})

class CordicRocketConfig extends Config(
  new WithNoMonitors ++ // <--- Ép hệ thống tắt toàn bộ bộ kiểm tra Monitor
  new WithCordicAccelerator ++
  new freechips.rocketchip.rocket.WithNHugeCores(1) ++
  new chipyard.config.AbstractConfig
)